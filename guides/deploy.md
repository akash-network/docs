...
| `AKASH_CHAIN_ID` | Chain ID of the Akash network connecting to. See [here](version.md). |
| `ACCOUNT_ADDRESS` | The address of your account.  See [here](wallet.md#account-address). |
| `KEY_NAME` | The name of the key you will be deploying from. See [here](wallet.md) if you haven't yet setup a key |
| `KEYRING_BACKEND` | Keyring backend to use for local keys. See [here](wallet.md) |

Verify you have correct `$AKASH_NODE`, that you have populated while [configuring the connection](https://github.com/ovrclk/docs/tree/49693ed5ec37b3928d8d28131d66293041132c77/guides/version/README.md) using `export AKASH_NODE=$(curl -s "$AKASH_NET/rpc-nodes.txt" | shuf -n 1)`.
...
Verify you have the key set up and your account has sufficient balances, see the [funding guide](funding.md) otherwise:
...
Please note the balance indicated is is denominated in uAKT \(AKT  _10^-6\), in the above example, the account has a balance of_ 93 AKT\*. We're now setup to deploy.

## Create The Deployment Configuration

Create a deployment configuration [deploy.yml](https://github.com/ovrclk/docs/tree/49693ed5ec37b3928d8d28131d66293041132c77/guides/deploy/deploy.yml) to deploy the `ovrclk/lunie-light` for [Lunie Light](https://github.com/ovrclk/lunie-light) Node app container using [SDL](../documentation/sdl.md):
...
You may use the sample deployment file as-is or modify it for your own needs as desscribed in our [SDL \(Stack Definition Language\)](https://github.com/ovrclk/docs/tree/49693ed5ec37b3928d8d28131d66293041132c77/guides/deploy/documentation/sdl/README.md) documentation. A typical modification would be to reference your own image instead of our demo app image.
...
In the above example, we can see that a lease is created using for _186 uakt_ or _0.00000186 AKT_ per block to execute the container.
...
You can access the application by visiting the hostnames mapped to your deployment. In above example, its [http://6veev7chcfmnclgqklegcc.provider4.akashdev.net](http://6veev7chcfmnclgqklegcc.provider4.akashdev.net)
...
where `$SERVICE_NAME` is the name of a service defined in your [SDL](../documentation/sdl.md).
