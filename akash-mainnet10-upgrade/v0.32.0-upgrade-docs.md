...
At the proposed block height, `cosmovisor` will automatically stop the current binary (v0.30.X), set the upgrade binary as the new current binary (v0.32.0), and then restart the node.\\
...
```shell
git clone --depth 1 --branch v0.32.0 https://github.com/akash-network/node
cd node
direnv allow
make release
```
