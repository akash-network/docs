## provider-services tx deployment group close

close a Deployment's specific Group

```
provider-services tx deployment group close [flags]
```

### Examples

```
akash tx deployment group-close --owner=[Account Address] --dseq=[uint64] --gseq=[uint32]
```
