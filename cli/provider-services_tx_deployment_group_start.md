## provider-services tx deployment group start

start a Deployment's specific Group

```
provider-services tx deployment group start [flags]
```

### Examples

```
akash tx deployment group pause --owner=[Account Address] --dseq=[uint64] --gseq=[uint32]
```
