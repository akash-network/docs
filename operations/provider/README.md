...
* A wallet funded with at least 50 AKT.
  ...
  * You can refer to [https://docs.akash.network/guides/funding](https://docs.akash.network/guides/funding) to purchase AKT and [https://docs.akash.network/guides/deploy](https://docs.akash.network/guides/deploy) to install Akash CLI wallet and read how to fund it.
...
As you may have noticed we have set `gvisor_enabled` to `true` in the kubespray inventory file, this will enable [gVisor](https://gvisor.dev/) support. gVisor limits the attack surface of the host. It also needs the `containerd` container engine. Modify `k8s-cluster.yml` and `etcd.yml` files to enable it:
...
The **cluster ingress host** is the publicly accessible hostname of the Kubernetes cluster. The Kubernetes cluster hosts an Ingress Controller, which is one way that leases in the cluster may be exposed to the outside world. It is currently recommended to [assign](https://github.com/ovrclk/docs/blob/master/guides/provider.md#Create-Kubernetes-Ingress-Controller) exactly one node in your cluster to the Ingress Controller role.
...
The container requesting 0.5 CPU, 2G RAM, 32G storage would be quoted for a price of `10 uakt` which is `$11.22` a month if we take 1 AKT price is `$2.56`. This way the Akash providers can always adjust the bid prices whenever they want, which creates great competition!
