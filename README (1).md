...
If you're new to Akash, start with the [**Getting Started guide**](cli/install.md) and go from there. Akash's community has written several more advanced guides for learning about Akash: a [node operator guide](broken-reference), a [validator guide](operations/validator.md), a [cloud provider guide](operations/build-a-cloud-provider/), and several [deployment guides](guides/) for running various apps on Akash.
