...
Either download the [Akash binary](v0.28.0-upgrade-docs.md#upgrade-details) or build it from source. We highly recommend using a pre-complied binary but provide instructions to build from source [here](v0.28.0-upgrade-docs.md#build-binary-from-source) in the rare event it would be necessary.
...
Create the folder for the upgrade (v0.30.0) - cloned in this [step](v0.28.0-upgrade-docs.md#common-steps-for-all-upgrade-options) - and copy the akash binary into the folder.
...
cp ./.cache/bin $HOME/.akash/cosmovisor/upgrades/v0.39.0/bin
```
...
Either download the [Akash binary](v0.28.0-upgrade-docs.md#upgrade-details) or build from source - completed in this [step](v0.28.0-upgrade-docs.md#common-steps-for-all-upgrade-options) - and ensure the `akash` binary has been updated:
