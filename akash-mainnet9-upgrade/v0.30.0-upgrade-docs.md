...
Either download the [Akash binary](v0.30.0-upgrade-docs.md#upgrade-details) or build it from source. We highly recommend using a pre-complied binary but provide instructions to build from source [here](v0.30.0-upgrade-docs.md#build-binary-from-source) in the rare event it would be necessary.
