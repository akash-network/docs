...
Either download the Akash binary (v0.28.0) or build it from source.  We highly recommend using a pre-complied binary but provide instructions to build from source [here](v0.24.0-upgrade-docs.md#build-binary-from-source) in the rare event it would be necessary.
...
Create the folder for the upgrade binary (v0.28.0) - cloned in this [step](v0.24.0-upgrade-docs.md#common-steps-for-all-upgrade-options) - and copy the akash binary into the folder.
...
Either download the Akash upgrade binary (v0.28.0) or build from source - completed in this [step](v0.24.0-upgrade-docs.md#common-steps-for-all-upgrade-options) - and ensure the `akash` binary has been updated:
