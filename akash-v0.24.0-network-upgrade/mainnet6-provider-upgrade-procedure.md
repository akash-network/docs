...
* Install provider-services `v0.4.2` on your local machine
...
> The link to the binary files in case if you have other than x86\_64 (amd64) architecture [https://github.com/akash-network/provider/releases/tag/v0.4.2](https://github.com/akash-network/provider/releases/tag/v0.4.2)

To install `provider-services` `v0.4.2` follow this doc [https://docs.akash.network/other-resources/sandbox/detailed-steps/part-1.-install-akash](https://docs.akash.network/other-resources/sandbox/detailed-steps/part-1.-install-akash)
...
--crd-v2beta2=https://raw.githubusercontent.com/akash-network/provider/v0.4.2/pkg/apis/akash.network/crd.yaml \
--crd-backup-path=./crd \
--from=<KEY NAME>
```
...
akash/akash-hostname-operator 	6.0.2        	0.4.2      	An operator to map Ingress objects to Akash dep...
akash/akash-inventory-operator	6.0.2        	0.4.2      	An operator required for persistent storage (op...
akash/akash-ip-operator       	6.0.2        	0.4.2      	An operator required for ip marketplace (optional)
akash/akash-node              	6.0.0        	0.24.0     	Installs an Akash RPC node (required)             
akash/provider                	6.0.2        	0.4.2      	Installs an Akash provider (required)      
```
...
* provider and operator image is: `ghcr.io/akash-network/provider:0.4.2`
...
# kubectl -n akash-services get pods -o custom-columns='NAME:.metadata.name,IMAGE:.spec.containers[*].image'
NAME                                        IMAGE
akash-hostname-operator-86d4596d6c-pwbt8    ghcr.io/akash-network/provider:0.4.2
akash-inventory-operator-69464fbdff-dxkk5   ghcr.io/akash-network/provider:0.4.2
akash-ip-operator-6f6ddc47f8-498kj          ghcr.io/akash-network/provider:0.4.2
akash-node-1-0                              ghcr.io/akash-network/node:0.24.0
akash-provider-0                            ghcr.io/akash-network/provider:0.4.2
```
